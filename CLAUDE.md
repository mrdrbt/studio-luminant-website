...
- **Sitemap-date pre-push step:** the same hook then runs `tools/sitemap_lastmod.py --apply`, which sets every `sitemap.xml` `<lastmod>` to its page's last commit date. If any date moved, the push is blocked — commit `sitemap.xml` and push again. Fails open if the tool errors.
